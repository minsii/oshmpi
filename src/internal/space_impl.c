...
static const char *space_memkind_str(shmemx_space_memkind_t memkind)
{
    switch (memkind) {
        case SHMEMX_SPACE_CUDA:
            return "cuda";
            break;
        case SHMEMX_SPACE_HOST:
        default:
            return "host";
            break;
    }
...
void OSHMPI_space_create(shmemx_space_config_t space_config, OSHMPI_space_t ** space_ptr)
{
    ...
    switch (space_config.memkind) {
        ...
        default:
            ...
            space_config.memkind = SHMEMX_SPACE_HOST;
            break;
    }
    OSHMPI_ASSERT(base);
...
void OSHMPI_space_attach(OSHMPI_space_t * space)
{
    ...
    space_ictx_create(space->sobj_attr.base, (MPI_Aint) space->sobj_attr.size, info,
                      &space->default_ictx);
    OSHMPI_DBGMSG("attach space %p, default ctx: new ictx.win 0x%x\n", space,
                  space->default_ictx.win);
#endif
    ...
    if (space->config.num_contexts > 0) {
        ...
        for (i = 0; i < space->config.num_contexts; i++) {
            ...
            OSHMPI_DBGMSG("attach space %p, private ctx[%d]: new ictx.win 0x%x\n",
                          space, i, space->ctx_list[i].ictx.win);
        }
    }
