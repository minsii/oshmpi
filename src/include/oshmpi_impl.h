...
typedef struct OSHMPI_sobj_attr {
    ...
    void *base;
    MPI_Aint size;
#if defined(OSHMPI_ENABLE_DYNAMIC_WIN) || defined(OSHMPI_ENABLE_RMA_ABS)
...
void OSHMPI_sobj_init_attr(OSHMPI_sobj_attr_t * sobj_attr, shmemx_space_memkind_t memkind,
                                                                 ...
                                                                 MPI_Aint * disp_ptr)
{
    ...
    {   /* If neither dynamic win nor rma_abs is enabled, we always use relative disp */
        *disp_ptr = (MPI_Aint) abs_addr - (MPI_Aint) sobj_attr->base;
    }
}
